...
export class SzCrossSourceResultsDataTable extends SzDataTable implements OnInit, OnDestroy {
    ...
    */
    ...
    /** Tracks which columns have been fully expanded via "Expand All". */
    private _expandedColumns = new Set<string>();

    /** Check if a column has been fully expanded. */
    public isColumnFullyExpanded(colKey: string): boolean {
      return this._expandedColumns.has(colKey);
    }

    /** Toggle expand/collapse all cells in a column. */
    public toggleExpandAllCellsInColumn(colKey: string) {
      if (this._expandedColumns.has(colKey)) {
        this.collapseAllCellsInColumn(colKey);
      } else {
        this.expandAllCellsInColumn(colKey);
      }
    }

    /** Collapse all cells in a column across all rows. */
    public collapseAllCellsInColumn(colKey: string) {
      if (!colKey || !this.tableRef?.nativeElement) return;
      this._expandedColumns.delete(colKey);
      const cssClass = `sz-dt-${colKey.toLowerCase().replace(/_/g, '-')}-column`;
      const cells = this.tableRef.nativeElement.querySelectorAll(`td.${cssClass}`);
      cells.forEach((td: Element) => {
        td.classList.remove('expanded');
        const tr = td.closest('tr');
        if (tr) {
          // Recalculate: if no expanded cells remain in this row, remove the CSS var
          const stillExpanded = tr.querySelectorAll('td.expanded');
          if (stillExpanded.length === 0) {
            tr.style.removeProperty('--expanded-cell-height');
          } else {
            // Recalc to tallest remaining expanded cell
            let maxHeight = 0;
            stillExpanded.forEach((el: Element) => {
              const content = el.querySelector('.cell-content') as HTMLElement;
              if (content) maxHeight = Math.max(maxHeight, content.scrollHeight);
            });
            if (maxHeight > 0) {
              tr.style.setProperty('--expanded-cell-height', `${maxHeight}px`);
            }
          }
        }
      });
    ...
    /** Expand all cells in a column across all rows. */
    public expandAllCellsInColumn(colKey: string) {
      if (!colKey || !this.tableRef?.nativeElement) return;
      this._expandedColumns.add(colKey);
      const cssClass = `sz-dt-${colKey.toLowerCase().replace(/_/g, '-')}-column`;
      const cells = this.tableRef.nativeElement.querySelectorAll(`td.${cssClass}`);
      cells.forEach((td: Element) => {
        ...
        const tr = td.closest('tr');
        if (tr) {
          // Find the tallest content in this row to set the CSS var
          let maxHeight = 0;
          tr.querySelectorAll('td.expanded .cell-content').forEach((el: Element) => {
            maxHeight = Math.max(maxHeight, (el as HTMLElement).scrollHeight);
          });
          if (maxHeight > 0) {
            tr.style.setProperty('--expanded-cell-height', `${maxHeight}px`);
          }
    ...
    /** Expand all cells in the row to their full content height. */
    public expandAllCellsInRow(cellElement?: HTMLElement) {
      if (!cellElement) return;
      const tr = cellElement.closest('tr');
      if (!tr) return;
      // Find the tallest cell content in the row
      let maxHeight = 0;
      const cells = tr.querySelectorAll('td.sz-dt-cell');
      cells.forEach((td: Element) => {
      ...
      // Expand all cells and set the CSS var to the tallest content
      cells.forEach((td: Element) => {
        td.classList.add('expanded');
      });
      if (maxHeight > 0) {
